...
import type { OrdemCompra } from '@/types/types';

const { getItems, updateItem, getItemById } = useDirectusItems();

const results = ref<OrdemCompra[]>([]);
const router = useRouter();

const actionApagar = async (id: number) => {
    await updateItem({ collection: 'ordem_compra', id: id, item: { status: 'draft' } });
    getResults();
};

const getResults = async () => {
    results.value = await getItems<OrdemCompra>({
        collection: "ordem_compra", params: {
            filter: {
    ...
    <section>
        <div class="mx-auto max-w-7xl">
            <div class="mx-auto flex items-center justify-between">
                <h2 class="text-2xl font-bold text-[#1B5870]">Ordem de Compra</h2>
                    ...
                    <table class="table-striped table-hover">
                        ...
                        <tbody>
                            <template v-for="item in results" :key="item.id">
                                <tr>
                                    <td class="whitespace-nowrap">{{ $filters.formatDate(item.date_created) }}</td>
                                    <td>
                                    ...
                                    <td class="whitespace-nowrap">{{ $filters.moedaBRL(item.valor_total) }}</td>
                                    <td>
                                        <el-dropdown size="large" type="primary">
                                            ...
                                            <template #dropdown>
                                                <el-dropdown-menu>
                                                    <el-dropdown-item
                                                        @click="router.push('/compras/ordem-compra/' + item.id)">
                                                        Editar
                                                    </el-dropdown-item>
