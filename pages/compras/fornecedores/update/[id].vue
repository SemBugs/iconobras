...
const salvar = async () => {
    ...
    await updateItem<any>({
        ...
        id: route.params.id,
        item: form.value,
    });//'fornecedor', route.params.id, form.value
    ElNotification({
        title: 'Sucesso!',
...
onBeforeMount(async () => {
    form.value = await getItemById({ collection: 'fornecedor', id: route.params.id });
});
</script>
...
<template>
    <div class="mx-auto max-w-5xl">
        <div class="mx-auto flex items-center justify-between">
            <h2 class="text-2xl font-bold text-[#1B5870]">Editar fornecedor</h2>
        </div>
        ...
        <div class="mt-5 flex justify-end space-x-4">
            <NuxtLink to="/compras/fornecedores" class="btn btn-dark"> Cancelar </NuxtLink>
