<template>
    <div class="mx-auto bg-white panel">
        <div class="mx-auto flex items-center justify-between">
            <h2 class="text-2xl font-bold">Tipos de Bombas de Concreto</h2>
            <button @click.prevent="isCreateBombasLancasOpen = true" class="btn btn-primary px-4 py-2" type="button">
            ...
            <table>
                <thead class="text-boton text-normal">
                    <tr>
                        <th>Bomba</th>
                        <th>Taxa Mínima</th>
                        <th>Mínimo Bombeado</th>
                        <th>Taxa M³ Faltante</th>
                        <th>Preço por m³</th>
                        <th>Taxa Extra</th>
                        <th>Código</th>
                        <th>Ações</th>
                ...
                <tbody class="bg-white">
                    <tr v-for="(item, index) in results" :key="index">
                        ...
                        <td>
                            {{ item.taxa_m3_faltante }}
                        </td>
                        <td>
                            {{ $filters.moedaBRL(item.preco_m3) }}
                        </td>
                        <td>
                            {{ item.taxa_extra }}
                        </td>
        ...
        <!-- Modal -->
        <TransitionRoot appear :show="isCreateBombasLancasOpen" as="template">
                ...
                <div class="flex min-h-full items-center justify-center px-4 py-8">
                    <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                        ...
                        <DialogPanel
                            ...
                            <CreateConcretoModal @close="closeEscorasModal" :tipo="selectedCategory" :data="form" />
                        </DialogPanel>
                    </TransitionChild>
                </div>
            </div>
...
import CreateConcretoModal from './CreateBombasLancasModal.vue';

const { getItems, deleteItem } = useDirectusItems();

const isCreateBombasLancasOpen = ref(false);
const form = ref({});
...
const editEscoras = (item) => {
    console.log(item)
    form.value = item;
    isCreateBombasLancasOpen.value = true;
};

const closeEscorasModal = () => {
    isCreateBombasLancasOpen.value = false
    form.value = {}
    changeTab(0)
};
</script>