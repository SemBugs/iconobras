<template>
    <div class="mx-auto bg-white panel">
        <div class="mx-auto flex items-center justify-between">
            <h2 class="text-2xl font-bold">Concreto</h2>
            <button @click.prevent="isCreateConcretoOpen = true" class="btn btn-primary px-4 py-2" type="button">
            ...
            <table>
                ...
                <tbody class="bg-white">
                    <tr v-for="(item, index) in results" :key="index">
                        ...
                        <td>
                            {{ $filters.moedaBRL(item.preco_m_cubico) }}
                        </td>
                        <td>
                            {{ item.codigo }}
                        </td>
                        <td class="w-[60px]">
                            <div class="flex gap-5">
                                <button @click="editConcreto(item)">
    ...
    <!-- Modal -->
    <TransitionRoot appear :show="isCreateConcretoOpen" as="template">
                ...
                <div class="flex min-h-full items-center justify-center px-4 py-8">
                    <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                        ...
                        <DialogPanel
                            ...
                            <CreateConcretoModal @close="closeConcretoModal" :tipo="'concreto'" :data="form" />
                        </DialogPanel>
                    </TransitionChild>
                </div>
            </div>
...
import CreateConcretoModal from './CreateConcretoModal.vue';

const { getItems, deleteItem } = useDirectusItems();

const isCreateConcretoOpen = ref(false);
const form = ref({});

const results = ref([{
    id: 1,
    margem_slamp: 'margem_slamp',
    tava_min_bombeamento: 'tava_min_bombeamento',
    limite_m_bombeado_sem_taxa: 'limite_m_bombeado_sem_taxa',
    taxa_bombeamento_m_excedente: 'taxa_bombeamento_m_excedente',
    taxa_min_bombeamento: 'taxa_min_bombeamento',
    limite_m_bombeado_sem_taxa_extra: 'limite_m_bombeado_sem_taxa_extra',
    valor_incluso: 'valor_incluso',
    valor_cobrado_a_parte: 'valor_cobrado_a_parte',
    nao_trabalha_sarramento: 'nao_trabalha_sarramento',
    taxa_minima_sarramento: 'taxa_minima_sarramento',
    taxa_sarramento_m_excedente: 'taxa_sarramento_m_excedente',
    fck_concreto: 'fck_concreto',
    tipo_agregado: 'tipo_agregado',
    tipo_slamp: 'tipo_slamp',
    especificao: 'especificao',
    polido: 'polido',
    codigo:'codigo',
    preco_m_cubico: parseFloat(150.7).toFixed(2)
},
{
    id: 2,
    margem_slamp: 'margem_slamp',
    ...
    limite_m_bombeado_sem_taxa_extra: 'limite_m_bombeado_sem_taxa_extra',
    valor_incluso: true,
    valor_cobrado_a_parte: true,
    nao_trabalha_sarramento: true,
    taxa_minima_sarramento: 'taxa_minima_sarramento',
    taxa_sarramento_m_excedente: 'taxa_sarramento_m_excedente',
    fck_concreto: 'fck_concreto',
    tipo_agregado: 'tipo_agregado',
    ...
    polido: 'polido',
    codigo:'codigo',
    preco_m_cubico: 150.7,
},
]);

const editConcreto = (item) => {
    console.log(item)
    form.value = item;
    isCreateConcretoOpen.value = true;
};

const closeConcretoModal = () => {
    isCreateConcretoOpen.value = false
    form.value = {}
    changeTab(0)
};
</script>