...
const formFields = ref([
    createField('Tipo de Aço', props.data.tipo_peca, 'text', false),
    createField('Diamêtro do aço', props.data.diamentro_do_aco, 'select', false),
    createField('Comprimento da barra', props.data.comprimento_m_linear, 'text', false),
    createField('Preço da barra', parseFloat(props.data.preco_m_linear).toFixed(2), 'text', false),
]);
const addFields = () => {
    formFields.value.push(createField('Tipo de Aço', props.data.tipo_peca, 'text', false))
    formFields.value.push(createField('Diamêtro do aço', props.data.diamentro_do_aco, 'select', false))
    formFields.value.push(createField('Comprimento da barra', props.data.comprimento_m_linear, 'select', false))
    formFields.value.push(createField('Preço da barra', parseFloat(props.data.preco_m_linear).toFixed(2), 'text', true))
};

const remove = () => {
    let lastIndex = formFields.length;

    formFields.value.splice(lastIndex - 1, 4);
}
    ...
    <el-tabs v-model="activeName" type="card" class="demo-tabs" @tab-click="handleClick">
        <el-tab-pane label="Geral" name="geral">
            <h2 class="text-[#FD4C0C] font-bold mb-2">
                {{ data.id ? 'Editar Distribuidoras de Aço' : 'Cadastro de Distribuidoras de Aço ' }}
            </h2>
