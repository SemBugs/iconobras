<template>
    <section>
        ...
        <div class="mx-auto bg-white panel my-4 ">
            <select class="w-full color border border-blue-400 h-10 text-center rounded text-blue-400 text-base cursor-pointer">
                <option>Aço 1</option>
                <option>Aço 2</option>
                <option>Aço 3</option>
            </select>
        </div>
        <div class="mx-auto bg-white panel mb-2">
            <div class="mx-auto flex items-center justify-between">
                <h2 class="text-2xl font-bold">Barras e Vergalhões Cadastrados</h2>
                <button @click.prevent="isCreateVergalhoesBarrasOpen = true" class="btn btn-primary px-4 py-2"
                    type="button">
            ...
            <div class="table-responsive mt-12 rounded-2xl bg-[#f3f3f3]">
                <table>
                    <thead class="text-boton text-normal">
                        <tr>
                            <th>Diâmentro do Aço</th>
                            <th>Comprimento M. Linear</th>
                            <th>Preço/M. Linear</th>
                            <th>Código</th>
                            <th>Ações</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white">
                        <tr v-for="(item, index) in results" :key="index">
                            <td>
                                {{ item.diamentro_do_aco }}
                            </td>
                            <td>
                                {{ item.comprimento_m_linear }}
                            </td>
                            <td>
                            ...
                            <td class="w-[60px]">
                                <div class="flex gap-5">
                                    <button @click="editVergalhoes(item)">
        ...
        <!-- Modal -->
        <TransitionRoot appear :show="isCreateVergalhoesBarrasOpen" as="template">
                    ...
                    <div class="flex min-h-full items-center justify-center px-4 py-8">
                        <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                            ...
                            <DialogPanel
                                ...
                                <CreateConcretoModal @close="closeVergalhoesBarrasModal" :tipo="selectedCategory"
                                    :data="form" />
                            </DialogPanel>
                        </TransitionChild>
                    </div>
...
import CreateConcretoModal from './CreateVergalhoesBarrasModal.vue';

const { getItems, deleteItem } = useDirectusItems();

const isCreateVergalhoesBarrasOpen = ref(false);
const form = ref({});

const results = ref([
    {
        id: "1",
        diamentro_do_aco: "testes",
        comprimento_m_linear: "testes",
        preco_m_linear: "120.20",
        codigo: "testes"
    },

]);

const editVergalhoes = (item) => {
    form.value = item;
    isCreateVergalhoesBarrasOpen.value = true;
};

const closeVergalhoesBarrasModal = () => {
    isCreateVergalhoesBarrasOpen.value = false
    form.value = {}
    changeTab(0)
};
</script>