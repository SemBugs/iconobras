...
const form = ref({
    ...
    "malha": props.data.malha,
    "diamentro_aco":props.data.diamentro_aco,
...
const formFields = ref([
    createField('Modelo Malha Diamêtro Painel', props.data.tipo_peca, 'select', false),
    createField('Preço da Painel', parseFloat(props.data.preco_m_linear).toFixed(2), 'text', false),
]);
const addFields = () => {
    formFields.value.push(createField('Modelo Malha Diamêtro Painel', props.data.tipo_peca, 'text', false))
    formFields.value.push(createField('Preço da Painel', parseFloat(props.data.preco_m_linear).toFixed(2), 'text', true))
...
const remove = () => {
    ...
    formFields.value.splice(lastIndex-2, 2);
    console.log(formFields.value)
    ...
    <el-tabs v-model="activeName" type="card" class="demo-tabs">
        <el-tab-pane label="Geral" name="geral">
            <h2 class="text-[#FD4C0C] font-bold mb-2">
                {{ data.id ? 'Editar Aço Avulso' : 'Cadastro de Aço Avulso' }}
            </h2>
                ...
                <div v-for="(field, index ) in formFields" :key="index">
                    <div class="col-span-1" :id="'escoras' + index">
                        ...
                        <div v-else class="flex flex-1">
                            ...
                            <button v-if="field.btnDelete && index > 2" @click.prevent="remove" class="btn -ma-0 px-4 py-2"
                                type="button">
