<template>
    <div class="mx-auto bg-white panel">
        <div class="mx-auto flex items-center justify-between">
            <h2 class="text-2xl font-bold">Pisos Metálicos</h2>
            <button @click.prevent="isCreatePisosOpen = true" class="btn btn-primary px-4 py-2" type="button">
        ...
        <!-- Modal -->
        <TransitionRoot appear :show="isCreatePisosOpen" as="template">
                ...
                <div class="flex min-h-full items-center justify-center px-4 py-8">
                    <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                        ...
                        <DialogPanel
                            ...
                            <CreateConcretoModal @close="closePisosModal" :tipo="selectedCategory" :data="form" />
                        </DialogPanel>
                    </TransitionChild>
                </div>
            </div>
...
import CreateConcretoModal from './CreatePisosModal.vue';

const { getItems, deleteItem } = useDirectusItems();

const isCreatePisosOpen = ref(false);
const form = ref({});
...
const editPisos = (item) => {
    console.log(item)
    form.value = item;
    isCreatePisosOpen.value = true;
};

const closePisosModal = () => {
    isCreatePisosOpen.value = false
    form.value = {}
    changeTab(0)
};
</script>