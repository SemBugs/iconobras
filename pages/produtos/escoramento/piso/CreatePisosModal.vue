...
const form = ref({
    ...
    "dimensoes": props.data.diamentro,
...
const formFields = ref([
    createField('Tipo Peça', props.data.tipo_peca, 'select', false),
    createField('Dimensões', props.data.dimensoes, 'select', false),
    createField('Preço', parseFloat(props.data.preco).toFixed(2), 'money', false),
    createField('Codigo', props.data.preco_m_cubico, 'text', false)
]);
const addFields = () => {
    formFields.value.push(createField('Tipo Peça', props.data.tipo_peca, 'select', false))
    formFields.value.push(createField('dimensoes', props.data.dimensoes, 'select', false))
    formFields.value.push(createField('Preço', parseFloat(props.data.preco).toFixed(2), 'money', false))
    formFields.value.push(createField('Codigo', props.data.codigo, 'text', true))
};
...
onMounted(() => {
    if (props.data.id) {
        form.value = {
            "tipo_peca": props.data.tipo_peca,
            "dimensoes": props.data.dimensoes,
            "preco": props.data.preco,
            "codigo": props.data.codigo
        }
    }
    ...
    <el-tabs v-model="activeName" type="card" class="demo-tabs">
        <el-tab-pane label="Geral" name="geral">
            <h2 class="text-[#FD4C0C] font-bold mb-2">
                {{ data.id ? 'Editar Piso Metálico' : 'Cadastro de Piso Metálico' }}
            </h2>
                ...
                <div v-for="(field, index ) in formFields" :key="index">
                    <div class="col-span-1" :id="'escoras' + index">
                        ...
                        <div v-else class="flex flex-1">
                            <input v-if="field.type == 'money'" id="name" type="text" class="form-input" v-money="money" />
                            <input v-if="field.type == 'text'" id="name" type="text" class="form-input" />
                            <button v-if="field.btnDelete" @click.prevent="remove" class="btn -ma-0 px-4 py-2"
                                type="button">
