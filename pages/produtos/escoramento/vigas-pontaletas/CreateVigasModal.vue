...
const formFields = ref([
    createField('Tipo peça', props.data.diamentro, 'select', false),
    createField('Comprimento', props.data.diamentro, 'select', false),
    createField('Preço', parseFloat(props.data.preco).toFixed(2), 'text', false),
]);
const addFields = () => {
    formFields.value.push(createField('Tipo peça', props.data.diamentro, 'select', false))
    formFields.value.push(createField('Comprimento', props.data.diamentro, 'select', false))
    ...
    <el-tabs v-model="activeName" type="card" class="demo-tabs" @tab-click="handleClick">
        <el-tab-pane label="Geral" name="geral">
            <h2 class="text-[#FD4C0C] font-bold mb-2">
                {{ data.id ? 'Editar Vigas e Perfis' : 'Cadastro de Vigas e Perfis' }}
            </h2>
