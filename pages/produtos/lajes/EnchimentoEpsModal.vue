...
const getItens = async () => {
    ...
    results.value = [];
    const filters = {"produto": { "_eq": `${props.data.id}` }}
    results.value = await getItems({ collection: 'enchimento',
        params: {
            "filter": filters
        }
    });
    ...
    <div>
        <div v-if="newItem">
            <div class="mx-auto mb-10 flex max-w-7xl items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-[#1B5870]">Cadastrar Enchimento Isopor</h2>
                    <p class="mt-2 text-sm">Laje Treliça / EPS - {{ data?.tipo_trelica?.nome }}</p>
                </div>
                <div>
                    <button
            ...
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-8">
                <div class="col-span-4">
                    <label class="text-sm" for="name">Marca do Isopor</label>
                    <input id="name" type="text" class="form-input" v-model="form.marca" />
                </div>
                <div class="col-span-4">
                    <label class="text-sm" for="name">Tipo de Densidade </label>
            ...
            <div class="mx-auto my-6 max-w-7xl items-center justify-between">
                <h2 class="text-2xl font-bold text-[#1B5870]">Dimensões da Peça do Isopor</h2>
                <p class="text-xs">Coloque as medidas da peça conforme ilustração</p>
            </div>

            <icon-isopor-dimensoes />
        ...
        <div v-if="!newItem" class="mx-auto max-w-7xl items-center justify-between flex">
            <h2 class="text-2xl font-bold text-[#1B5870]">Listagem do Enchimento Isopor</h2>
        ...
        <div v-if="!newItem" class="table-responsive mt-12 rounded-2xl bg-[#f3f3f3]">
            <table>
                ...
                <tbody class="bg-white">
                    <tr v-for="(item, index) in results" :key="index">
                        <td>{{ item.marca }}</td>
                        <td>{{ item.a }} x {{ item.b }} x {{ item.c }}</td>
