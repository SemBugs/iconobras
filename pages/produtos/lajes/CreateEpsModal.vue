<script setup lang="ts">
    ...
    const { createItem, getItems, updateItem,getSingletonItem } = useDirectusItems();
...
<template>
    <div>
        <div class="mx-auto mb-5 max-w-7xl items-center justify-between">
            <h2 class="text-2xl font-bold text-[#1B5870]">
                {{ data.id ? 'Editar EPS- Isopor' : 'Cadastrar EPS- Isopor' }}
            </h2>
            ...
            <div class="flex gap-2">
                <p class="mb-2 mt-2 text-sm">
                    ...
                    <span v-if="form.tipo_trelica" class="mb-2 mt-2 text-sm"> / EPS - {{ getTrelica(form.tipo_trelica) }} </span>
                </p>
            </div>
        </div>
        ...
        <el-tabs v-model="activeName" type="card" class="demo-tabs" @tab-click="handleClick">
                ...
                <div class="grid flex-1 grid-cols-1 gap-5 sm:grid-cols-6">
                    <div class="col-span-3">
                        ...
                        <select id="origem_mercadoria" class="form-select text-white-dark" v-model="form.origem_mercadoria">
                            <option v-for="(item, index) in tipo_trelica" :key="index" :value="item.value">{{ item.nome }}</option>
                        </select>
                    </div>
                    <div class="col-span-3">
                        <label class="text-xs placeholder:text-xs" for="name">Situação Tributária</label>
                        <select id="situacao_tributaria" class="form-select text-white-dark" v-model="form.situacao_tributaria">
                            <option v-for="(item, index) in tipo_trelica" :key="index" :value="item.value">{{ item.nome }}</option>
                        </select>
                    </div>
                </div>
            ...
            <el-tab-pane label="IPI" name="ipi">
                <div class="grid flex-1 grid-cols-1 gap-5 sm:grid-cols-6">
                    <div class="col-span-3">
                        <label class="text-xs placeholder:text-xs" for="name">Origem da Mercadoria</label>
                        <select id="origem_mercadoria" class="form-select text-white-dark" v-model="form.ipi_origem_mercadoria">
                            <option v-for="(item, index) in tipo_trelica" :key="index" :value="item.value">{{ item.nome }}</option>
                        </select>
                    </div>
                    <div class="col-span-3">
            ...
            <el-tab-pane label="PIS" name="pis">
                <div class="grid flex-1 grid-cols-1 gap-5 sm:grid-cols-6">
                    <div class="col-span-6">
                        <label class="text-xs placeholder:text-xs" for="name">Código da situação tributária do PIS (CST)</label>
            ...
            <el-tab-pane label="COFINS" name="cofins">
                <div class="grid flex-1 grid-cols-1 gap-5 sm:grid-cols-6">
                    <div class="col-span-6">
                        <label class="text-xs placeholder:text-xs" for="name">Código da situação tributária do COFINS (CST)</label>
    ...
    <div class="mt-5 flex justify-end space-x-4">
        <button @click.prevent="cancel" class="btn btn-dark"><icon-archive class="h-5 w-5 shrink-0 ltr:mr-2 rtl:ml-2" /> Cancelar</button>
