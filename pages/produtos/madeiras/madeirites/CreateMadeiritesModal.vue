...
const form = ref({
    "tipo_madeira": props.data.tipo_madeira,
    "comprimento": props.data.comprimento,
    "espessura":props.data.espessura,
...
const createField = (label, valueField, type, btnDelete) => {
    return { label, valueField, type, btnDelete };
};

const formFields = ref([
    createField('Tipo de Madeira', props.data.tipo_madeira, 'select', false),
    createField('Comprimento', props.data.comprimento, 'select', false),
    createField('Espessura', props.data.espessura, 'select', false),
    createField('Preço da peça', parseFloat(props.data.preco).toFixed(2), 'text', false),
]);
const addFields = () => {
    formFields.value.push(createField('Modelo Malha Diamêtro Painel', props.data.tipo_peca, 'text', false))
    formFields.value.push(createField('Comprimento', props.data.comprimento, 'text', false))
    formFields.value.push(createField('Espessura', props.data.espessura, 'text', false))
    formFields.value.push(createField('Preço da peça', parseFloat(props.data.preco).toFixed(2), 'text', true))
};

const remove = () => {
    let lastIndex = formFields.value.length;
    formFields.value.splice(lastIndex-4, 4);
    ...
    <el-tabs v-model="activeName" type="card" class="demo-tabs">
        <el-tab-pane label="Geral" name="geral">
            <h2 class="text-[#FD4C0C] font-bold mb-2">
                {{ data.id ? 'Editar Madeirites' : 'Cadastro de Madeirites' }}
            </h2>
                ...
                <div v-for="(field, index ) in formFields" :key="index">
                    <div class="col-span-1" :id="'escoras' + index">
                        ...
                        <select v-if="field.type == 'select'" id="fck_concreto" class="form-select text-white-dark"
                            v-model="field.valueField"> 
                            <option v-for="(item, index) in enchimento" :key="index" :value="item.id">
                                {{ item.nome }}
                            </option>
                        </select>
                        <div v-else class="flex flex-1">
                            <input id="name" type="text" class="form-input" v-money="money" v-model="field.valueField" />
                            <button v-if="field.btnDelete && index > 4" @click.prevent="remove" class="btn -ma-0 px-4 py-2"
                                type="button">
