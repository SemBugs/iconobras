<template>
    <div class="mx-auto bg-white panel">
        <div class="mx-auto flex items-center justify-between">
            <h2 class="text-2xl font-bold">Eucalipto</h2>
            <button @click.prevent="isCreateEucaliptoOpen = true" class="btn btn-primary px-4 py-2" type="button">
                    ...
                    <tr v-for="(item, index) in results" :key="index">
                        ...
                        <td class="w-[60px]">
                            <div class="flex gap-5">
                                <button @click="editEucalipto(item)">
        ...
        <!-- Modal -->
        <TransitionRoot appear :show="isCreateEucaliptoOpen" as="template">
                ...
                <div class="flex min-h-full items-center justify-center px-4 py-8">
                    <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                        ...
                        <DialogPanel
                            ...
                            <CreateEucaliptoModal @close="closeEucaliptoModal" :tipo="selectedCategory" :data="form" />
                        </DialogPanel>
                    </TransitionChild>
                </div>
            </div>
...
import CreateEucaliptoModal from './CreateEucaliptosModal.vue';

const { getItems, deleteItem } = useDirectusItems();

const isCreateEucaliptoOpen = ref(false);
const form = ref({});
...
const editEucalipto = (item) => {
    form.value = item;
    isCreateEucaliptoOpen.value = true;
};

const closeEucaliptoModal = () => {
    isCreateEucaliptoOpen.value = false
    form.value = {}
    changeTab(0)
};
</script>