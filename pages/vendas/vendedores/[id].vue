...
import type { Empresa, Vendedor } from '@/types/types';
...
const salvar = async () => {
    if (route.params.id === 'create') {
        ...
        await createItems({ collection: 'vendedor', items: form.value });
        ElNotification({
            title: 'Sucesso!',
    ...
    router.push('/vendas/vendedores');
};

onBeforeMount(async () => {
    if (route.params.id !== 'create') {
        form.value = await getItemById({ collection: 'vendedor', id: route.params.id });
    }
});
</script>
    ...
    <div class="mx-auto max-w-5xl">
        <div class="mx-auto flex items-center justify-between">
            <h2 class="text-2xl font-bold text-[#1B5870]">
                {{ route.params.id === 'create' ? 'Novo' : 'Editar' }} vendedor
            </h2>
        </div>
        ...
        <div class="mt-6">
            <div class="grid flex-1 grid-cols-1 gap-5 pt-3 sm:grid-cols-8">
                ...
                <div class="col-span-2">
                    <label class="text-xs placeholder:text-xs" for="inscrição-estadual">Comissão</label>
                    <input v-model="form.comissao" id="inscrição-estadual" type="text" class="form-input" />
                </div>
            </div>
        </div>
        ...
        <div class="mt-10">
            ...
            <div class="grid flex-1 grid-cols-1 gap-5 pt-3 sm:grid-cols-8">
                ...
                <div class="col-span-2">
                    ...
                    <input id="bairro" type="text" class="form-input" />
                </div>
                <div class="col-span-2">
        ...
        <div class="mt-5 flex justify-end space-x-4">
            <NuxtLink to="/vendas/vendedores" class="btn btn-dark"> Cancelar </NuxtLink>
