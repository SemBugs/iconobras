...
const auth = async () => {
    try {
        ...
        const empresas = await getItems({ collection: 'empresa' })

        // set empresas in localstorage
        localStorage.setItem('empresas', JSON.stringify(empresas));

        // set the first empresa in localstorage
        localStorage.setItem('empresa', JSON.stringify(empresas[0]));

        await router.push('/company-selector');
    } catch (error: any) {
        for (const e of error.response._data.errors) {
        ...
        <header
            ...
            <nav aria-label="Header Navigation"
                ...
                <div class="my-4 flex items-center space-x-6 space-y-2 lg:my-0 lg:ml-auto lg:space-x-8 lg:space-y-0">
                    <a href="#" title=""
                        class="whitespace-nowrap rounded-xl border border-red-500 px-5 py-2 font-medium text-red-500">Entrar</a>
                </div>
            </nav>
        </header>
            ...
            <div
                ...
                <div
                    ...
                    <div
                        ...
                        <div class="lg:p-6">
                            <div
                                class="mb-8 flex flex-shrink-0 flex-grow-0 items-center justify-center overflow-hidden">
                            </div>
                            <h4 class="mb-2 text-2xl font-bold text-[#2F2F2F]">Entre com os seus dados</h4>
                            <p class="mb-6 font-medium text-[#2F2F2F]">Faça seu primeiro pedido online!</p>

                            <form @submit.prevent="auth()" id="" class="mb-4" action="#" method="POST">
                                <div class="mb-4">
