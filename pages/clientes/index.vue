...
const actionApagar = async (id: number) => {
    await updateItem({ collection: 'cliente', id: id, item: { status: 'draft' } });
    getResults();
};

const getResults = async () => {
    results.value = await getItems<any>({
        collection: "cliente", params: {
            filter: {
    ...
    <section>
        <div class="mx-auto max-w-7xl">
            <div class="mx-auto flex items-center justify-between">
                <h2 class="text-2xl font-bold text-[#1B5870]">Clientes</h2>
                                    ...
                                    <td>
                                        <el-dropdown size="large" type="primary">
                                            ...
                                            <template #dropdown>
                                                <el-dropdown-menu>
                                                    <el-dropdown-item
                                                        @click="router.push('/clientes/update/' + item.id)">
                                                        Editar
                                                    </el-dropdown-item>
